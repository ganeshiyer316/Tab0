...
async function captureCurrentTabs() {
  try {
    ...
    // Improved error handling for service worker issues
    if (error && error.message && error.message.includes('No SW')) {
      console.error('Service worker initialization error. This is expected during development or first load.');
    } else {
